...
class TreeNode:
    ...
    def train(
        ...
        # the targets is.
        elif depth_left == 0 or len(features.columns) == 0:
...
class DecisionTree:
    ...
    def train(
        ...
        self.gain_func = None
        if   gain == "entropy":
            gain_func = DecisionTree.gen_gain_func(DecisionTree.entropy)
        elif gain == "majority error":
            gain_func = DecisionTree.gen_gain_func(DecisionTree.maj_err)
        elif gain == "gini":
            gain_func = DecisionTree.gen_gain_func(DecisionTree.gini)
        else:
    ...
    def gen_gain_func(entropy_func):
        ...
        def gains_func(features, target, weights):
            ...
            targ_ent = entropy_func(target) * sum(weights)
            ...
            for column in features.columns:
                ...
                if features[column].dtype in ["int64"]:
                    ...
                    top_entr = entropy_func(top_targ) if top_prob > 0 else targ_ent * 0.5
                    
                    bot_prob = sum(bot_wght) / sum(weights)
                    bot_entr = entropy_func(bot_targ) if bot_prob > 0 else targ_ent * 0.5
                    
                    col_gain -= (bot_entr * bot_prob
                               + top_entr * top_prob)
